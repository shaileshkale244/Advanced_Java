...
public class UserDaoImpl implements UserDao {
	...
	public String assignAddress(Address address, Long userId) {
		...
		try {
			...
			if (user != null) {
				...
				// added session.persist here explicitly to ret adr id to the user n not for
				// inserting a rec.
	...
	public List<User> getUserNamesByCity(String city1) {
		...
		Transaction tx = session.beginTransaction();
		try {
			users = session.createQuery(jpql, User.class).setParameter("city", city1).getResultList();
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
